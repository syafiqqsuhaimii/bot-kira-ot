...
def start(message):
    ...
    markup.add("Weekday","Weekend","Public Holiday")
    bot.send_message(
        message.chat.id,
...
def set_rate(message):
    ...
    bot.send_message(message.chat.id,f"✅ Rate OT disetkan kepada RM {rate:.2f}/jam.\nSila pilih jenis OT:", reply_markup=types.ReplyKeyboardMarkup(
        row_width=3, resize_keyboard=True
    ).add("Weekday","Weekend","Public Holiday"))

# Button OT type
@bot.message_handler(func=lambda m: m.text in ["Weekday","Weekend","Public Holiday"])
def pilih_ot(message):
    ...
    if jenis=="weekday":
        ...
        bot.send_message(chat_id,"Masukkan bilangan hari kerja weekend (contoh: 2 hari)")
    else:
